...
The Sherwood number reported by this script is hydraulic-diameter based:
    Sh_Dh = 2 Sh_l,
where Sh_l is based on the plate spacing b and Dh = 2b for wide parallel
plates. (b is the notation used in the README and sherwood_fit.py for this
geometry's length scale — the full gap, not the half-gap "a" used for the
two-wall geometry.)
...
# Conservative global bound for one-reactive-wall, hydraulic-Dh Sh:
...
def sherwood_gap_from_integrals(Da, phi_w, Im):
    ...
    Here l = b is the spacing between the inert and reactive plates.
    """
    denom = Im - phi_w
    if abs(denom) < 1e-300:
...
def plot_all_maps(Pe_tab, Da_tab, Sh_tab, chi_tab, Le_tab, prefix="pp_onewall_pois_fast"):
    ...
    plot_map(
        ...
        Da_tab,
        Le_tab,
        title=r"Entrance length $L_e/b \sim 1/(\beta_2-\beta_1)$",
        cbar_label=r"$L_e/b$",
...
def run_parallel_grid(
    ...
    output_dir="data",
    sh_filename="Sh_oneplate_pois.txt",
    chi_filename="chi_oneplate_pois.txt",
    le_filename="Le_oneplate_pois.txt",
    n_workers=None,
    beta_max=10.0,
    growth=1.12,
    make_plots=True,
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Parallel (Pe, Da) eigenvalue sweep for one-wall parallel plates."
    )
    parser.add_argument(
        "--workers",
        type=int,
