...
Geometry
--------
Parallel plates with TWO reactive surfaces, solved on the symmetric half-gap:
    rho = y/a in [0, 1]
    rho = 0 : symmetry plane
    rho = 1 : reactive wall
...
Pe = ubar a / D
Da = k a / D
uhat(rho) = (3/2) (1 - rho^2), normalized so int_0^1 uhat drho = 1.

The Sherwood number reported by this script is hydraulic-diameter based:
    Sh_Dh = 4 Sh_l,
where Sh_l is based on the half-gap a and Dh = 4a for parallel plates.
...
def velocity_profile(rho):
    """Mean-normalized plane Poiseuille profile on the half-gap."""
    rho = np.asarray(rho, dtype=float)
    return 1.5 * (1.0 - rho**2)


def graetz_ode_plates(rho, y, beta, Pe):
    """
    Transverse ODE system for parallel plates.
...
def graetz_ode_plates_aug(rho, y, beta, Pe):
    """
    Augmented ODE system for parallel plates.
...
def eigenfunction_with_integrals(
    ...
    If n_points is an integer, rho, phi, dphi arrays are also sampled.
...
def sherwood_halfgap_from_integrals(Da, phi_w, Im):
    """
    Stable half-gap-based Sherwood number:
...
def solve_beta1_and_sh_plates(
    ...
    Sh_l = sherwood_halfgap_from_integrals(Da, phi_w, Im)
...
def solve_point_safe(Pe, Da, beta_max=50.0, growth=1.12):
    ...
    """
    try:
        result = solve_beta1_and_sh_plates(
            Pe=Pe,
            Da=Da,
            beta_start=None,
            beta_max=beta_max,
...
def check_limits():
    """
    Quick spot checks against known limits.
    """
    tests = [
